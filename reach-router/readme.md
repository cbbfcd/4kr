...
利用了最新的 `Context API` 实现路由数据的向下分发，新 `API` 已经不受 `shouldComponentUpdate` 的干扰。

实现的本质还是一样的，通过在顶层组件中建立监听，一旦路由变化，触发监听，就会执行 `this.setState(() => newContext)` 这样所有消费者都会 `re-render`。

在消费者中再加一层判断拦截，如果命中该 `path`，则渲染，具有特色的地方就是：

1. `Router` 可以嵌套，支持相对路径，支持通配符
2. 利用异常来实现重定向，同时也兼容了 `16.x` 之前的版本
3. 递归的 `Router`，利用 `Context` 可以多级嵌套的机制，实现了嵌套路由

整体感觉作者是写了个 很牛逼的`demo` 来实现自己的想法，代码显得有点乱，里面的设计真心 `6`，值得学习，不过在大型项目中，性能堪忧。

此外截止到目前，这个项目已经积累了几十条 `PR`，一百多 `Issue` 没处理，看来，作者正在写下一代路由器了！

*PS*：看看别人的 `PR`，好多还是有点货的！