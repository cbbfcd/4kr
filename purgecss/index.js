...
class Purgecss {
    ...
    extractRawSelector(content: Array<RawContent>, extractors?: Array<ExtractorsObj>): Set<string> {
        ...
        // raw 就是文件字符串，extension 就是文件后缀名 eg: {raw: '<html>xxx</html>', extension: 'html'}
    ...
    getFileExtractor(filename: string, extractors: Array<ExtractorsObj> = []) {
        if (!extractors.length) return this.options.defaultExtractor // 默认的 extractor

        // 如果使用自定义的 extractor: [{extensions: [], extractor: class {}}]
    ...
    getSelectorsCss(selectors: Set<string>) {
        // http://api.postcss.org/AtRule.html#walk
        this.root.walk(node => {
            if (node.type === 'rule') {
            ...
            if (node.type === 'comment') {
                // 这里可以通过注释 /* purgecss ignore star*/ xxx /* purgecss ignore end*/ 这中间的 xxx 部分不会被消除掉
    ...
    evaluateRule(node: Object, selectors: Set<string>) {
        // 一般来说， rule 的前一个节点可能是一个注释
        const annotation = node.prev()
        if (this.ignore) return //一个锁
        // 如果注释节点包含 'purgecss ignore' 就干掉，这里有点标记清除的感觉
        if (
        ...
        // 父节点是 atrule 的情况，对 keyframes 类型的不处理
        if (node.parent && node.parent.type === 'atrule' && node.parent.name === 'keyframes') {
            return
        }

        let keepSelector = true
        // 这个库也很有意思，大概就是对选择器的字符串也做了解析
        // 在 npm run kit 跑一下下面的代码就知道了
        // postcssSelectorParser(ss => {
        //     ss.walk(s => {
        //         console.log(s);
        //     })
        // }).processSync('ul li:not(.ignore)')
        node.selector = selectorParser(selectorsParsed => {
            selectorsParsed.walk(selector => {
                ...
                if (selector.type === 'selector') {
                    ...
                    // :not(.ignore) 这里面的 selector `.ignore` 被忽略，不做处理
                    if (
                        selector.parent &&
                    ...
                    // FIXME: 这里有 _value 的情况啊，是不是有 bug？
                    ...
                    if (!keepSelector) {
                        // 如果设置了 rejected，就会把待删除的 selector 收集起来
