# 4kr

> 作者：波比小金刚



今年伊始，立下了 `flag` ，希望能够在未来几年内精读 `4000+` 优质开源库，并整理于此。



这也是名字 *4kr* 的由来：4000 `repos`。


---

之前有专门的博客，后来不想维护了，其实 `github` 就挺好的。

工作三年下来，前后端都做过，深感千里之行，始于基础。就在这里也插入对基础知识体系的整理、总结，希望有用于有缘人。


## 目录


### 2019-07 ~ ?

- [x] [on-change - 一个监控数据变化的小而精的项目](./on-change/readme.md)
- [x] [slow-json-stringify- 一个快的令人发指的`JSON.stringify()` 补充库](./slow-json-stringify/readme.md)
- [x] [morphdom - real dom diff 真的会慢吗?](./morphdom/readme.md)
- [x] [constate - 一个极其简单的React状态管理工具](./constate/readme.md)
- [x] [reach-router -- 听说是撕逼之后自立门户之作?](./reach-router/readme.md)
- [x] [purgecss -- 消除没有使用的 CSS，PostCss 真香！](./purgecss/readme.md)
- [ ] [Omi -- 当耐特又一作品](./omi/readme.md)
- [ ] [anu -- 司徒大大的作品，迷你React](./anu/readme.md)
- [ ] [redux -- 第三次拜读经典](./redux/readme.md)
- [ ] [redux-thunk -- 十几行代码一万多 star!](./redux-thunk/readme.md)
- [ ] [mobx -- 好像都出到 5 了，再看看吧](./mobx/readme.md)
- [ ] [immer -- 也是神作啊！](./immer/readme.md)
- [ ] [react-doc -- React 官方文档真的值得细细研究](./react-doc/readme.md)
- [ ] [react -- 这个，跪着也要看完](./react/readme.md)
- [ ] [memoize-one -- React 官方推荐，利用闭包做缓存对比](./memoize-one/readme.md)
- [ ] [wouter -- Hooks 加持的 1kb 的路由](./wouter/readme.md)
- [ ] [react-router -- 好像出到 v5.01 了，再看看有啥变化](./react-router/readme.md)
- [x] [copy-text-to-clipboard -- 如何写一个复制到剪贴板的方法](./copy-text-to-clipboard/readme.md)
- [x] [raf -- 这个也是高频出现的，了解其 polyfill 实现](./raf/readme.md)
- [x] [clone -- 学习如何写一个深度克隆](./clone/readme.md)
- [ ] [fat -- 最快的动画引擎](./fat/readme.md)
- [x] [goober -- 只有 1 kb 大小的mini styled-component](./goober/readme.md)
- [x] [babel-plugin-macros -- 定义babel插件新方式：宏](./macro/readme.md)
- [x] [typical -- 400byte的动画输入显示库](./typical/readme.md)


- [ ] [rust simple learn note](./rust/readme.md)

### course

- [ ] [计算机组成原理](./computer/readme.md)